...
if Report.all.empty?
  User.where("position in (?)", ["Leader", "Subleader", "Member"]).each do |user|
    50.times do |i|
      ...
      report = user.reports.new(title: "Report for some thing", description: "Let's wirte some thing here", report_category_id: ReportCategory.all[rand(ReportCategory.count)].id, report_date: date)
